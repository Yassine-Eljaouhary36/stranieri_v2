<template>
    <div class="meeting-panel">
        ...
        <div v-if="selectedOption === 'day'"
            style="display: flex; flex-direction: column;justify-content: center;align-items: center;" class="mb-3">
        ...
        <div v-if="filteredDay.active && selectedOption === 'day'">
            <div class="time-section">
                <h2>{{ transilation('am') }} ({{ getFirstTimeSlot(filteredDay.hours.filter(h => getAMorPM(h.hour) === 'AM')) }}
                    - {{ getLastTimeSlot(filteredDay.hours.filter(h => getAMorPM(h.hour) === 'AM')) }})</h2>
                <div class="time-slots">
                    <div v-for="hour in filteredDay.hours.filter(h => getAMorPM(h.hour) === 'AM')" :key="hour.hour" :class="{
                        'time-slot': true,
            ...
            <div class="time-section">
                <h2>{{ transilation('pm') }} ({{ getFirstTimeSlot(filteredDay.hours.filter(h => getAMorPM(h.hour) === 'PM')) }}
                    - {{ getLastTimeSlot(filteredDay.hours.filter(h => getAMorPM(h.hour) === 'PM')) }})</h2>
                <div class="time-slots">
                    <div class="time-slots">
            ...
            <div class="time-section">
                ...
                <div class="time-slots">
                    <div v-for="hour in filteredDay.hours.filter(h => getAMorPM(h.hour) === 'AM')" :key="hour.hour"
                        class="time-slot Unselectable" @click="inactiveDay">
                        {{ hour.hour }}
                    </div>
                </div>
            </div>
            ...
            <div class="calendar-week">
                <div v-for="day in generateWeek" :key="day.id" class="day">
                    ...
                    <div class="hours" v-if="!day.filtredDay.active">
                        <div v-for="hour in day.filtredDay.hours" :key="hour.hour" class="hour-btn Unselectable"
                            @click="inactiveDay">
                            {{ hour.hour }}
                        </div>
                    </div>
                    <div class="hours" v-if="day.filtredDay.active">
                        <div v-for="hour in day.filtredDay.hours" :key="hour.hour"
    ...
    methods: {
        ...
        addToCart() {
            ...
            if (!this.selectedTime) {
                Swal.fire(this.transilation('No_Meeting_Selected_Title'), this.transilation('No_Meeting_Selected_Body'), 'warning');
                return;
            } else {
                const existingItem = this.cart.find(item => item === this.selectedTime);
                if (existingItem) {
                    Swal.fire(this.transilation('Existed_Meeting_Title'), this.transilation('Existed_Meeting_Body'), 'info');
        ...
        inactiveDay() {
            Swal.fire(this.transilation('Reserved_Meeting_Title'), this.transilation('Reserved_Meeting_Body'), 'warning');
            this.selectedTime = null
            return;
        },
        saveCartToCookie() {
